...
class Trainer_step:
    ...
    def main(self, args):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            self.before_train_one_epoch(args, model, epoch)
            train_loss, train_acc1, train_acc5 = self.train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, num_classes, model_ema, scaler)
            if utils.is_main_process():
                tb_writer.add_scalar('train_loss', train_loss, epoch)
                tb_writer.add_scalar('train_acc1', train_acc1, epoch)
                tb_writer.add_scalar('train_acc5', train_acc5, epoch)
            ...
            lr_scheduler.step()
            self.before_test_one_epoch(args, model, epoch)
            test_loss, test_acc1, test_acc5 = self.evaluate(args, model, criterion, data_loader_test, num_classes=num_classes, device=device)
            if utils.is_main_process():
                tb_writer.add_scalar('test_loss', test_loss, epoch)
                tb_writer.add_scalar('test_acc1', test_acc1, epoch)
                tb_writer.add_scalar('test_acc5', test_acc5, epoch)
            if model_ema:
                ema_test_loss, ema_test_acc1, ema_test_acc5 = self.evaluate(args, model_ema, criterion, data_loader_test, device=device, num_classes=num_classes, log_suffix="EMA")
                if utils.is_main_process():
                    tb_writer.add_scalar('ema_test_loss', ema_test_loss, epoch)
                    tb_writer.add_scalar('ema_test_acc1', ema_test_acc1, epoch)
                    tb_writer.add_scalar('ema_test_acc5', ema_test_acc5, epoch)
    ...
    def get_args_parser(self, add_help=True):
        ...
        parser.add_argument("-T", "--time-step", type=int, help="time step")
        parser.add_argument("--data-path", default="/datasets01/imagenet_full_size/061417/", type=str, help="dataset path")
        parser.add_argument("--model", default="resnet18", type=str, help="model name")
        parser.add_argument("--device", default="cuda", type=str, help="device (Use cuda or cpu Default: cuda)")
        parser.add_argument(
        ...
        parser.add_argument("--mixup-alpha", default=0.0, type=float, help="mixup alpha (default: 0.2)")
        ...
        parser.add_argument("--auto-augment", default=None, type=str, help="auto augment policy (default: ta_wide)")
        parser.add_argument("--random-erase", default=0.0, type=float, help="random erasing probability (default: 0.1)")
        ...
        parser.add_argument("--disable-uda", action="store_true",
                            help="not set 'torch.use_deterministic_algorithms(True)', which can avoid the error raised by some functions that do not have a deterministic implementation")
        parser.add_argument("--present-transform", action="store_true",
                            help="use present transform when loading dataset")
